...
//!   authority assertions and recording debug status.
...
/// Informational claim recorded by the verifier.
...
pub struct AppraisalReport {
    ...
    /// Phase 5: Informational verifier claims.
...
pub fn appraise(
    ...
    let (verifier_checks, verifier_claims) = verify_preconditions(claims, expected_nonce);
...
fn verify_preconditions(
    claims: &OcpEatClaims,
    expected_nonce: Option<&str>,
) -> (Vec<VerifierCheck>, Vec<VerifierClaim>) {
    let mut checks = Vec::new();
    let mut claims_info = Vec::new();
    ...
    // 2. Debug status: record the claim, but do not fail appraisal for it.
    claims_info.push(VerifierClaim {
        name: "Debug status".into(),
        detail: format!("debug status is {}", claims.debug_status),
    });
