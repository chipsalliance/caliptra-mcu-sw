...
use super::decode::DecodedCoseSign1;
...
pub fn authenticate_signer(
    ...
    // Try x5chain
    if matches!(options.method, SignerIdMethod::X5chain | SignerIdMethod::Both) {
...
fn select_headers<'a>(
    ...
    match selection {
        ...
        HeaderSelection::Both => vec![decoded.protected_header(), decoded.unprotected_header()],
    }
}
...
fn build_chain(x5chain: Vec<Vec<u8>>, external_chain: &[u8]) -> CoseSign1Result<Vec<Vec<u8>>> {
    ...
    let mut chain_certs = split_der_certs(external_chain).map_err(CoseSign1Error::CertificateError)?;
