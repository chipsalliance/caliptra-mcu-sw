...
fn authenticate_pipeline<'a>(
    ...
    match ev.authenticate(&cert_chain_blob) {
        Ok(_) => {
            println!("  \x1b[32m✓\x1b[0m Evidence signer certificate chain authentication successful");
        }
        Err(e) => {
    ...
    // 6. Authenticate and verify signed reference value CoRIM files (if SIGNED_CORIM_PATH is set)
    let refval_corims = if let Ok(corims_path) = env::var(SIGNED_CORIM_PATH) {
...
fn run_appraise(args: &AppraiseArgs) {
    ...
    phase_banner("2", "Evidence Augmentation (Evidence Claimset Initialization)");
    ...
    if ctx.refval_corims.is_empty() {
        eprintln!("No CoRIM reference values loaded (set {} to enable appraisal)", SIGNED_CORIM_PATH);
        std::process::exit(1);
    }
    ...
    for result in &report.results {
        ...
        if result.passed() {
            println!("  \x1b[32m✓\x1b[0m {}{}{}", color, result.env_label, C_RESET);
        } else {
            println!("  \x1b[31m✗\x1b[0m {}{}{}", color, result.env_label, C_RESET);
        }
    ...
    if report.all_passed() {
        println!("  ATTESTATION RESULT: \x1b[32m✓ PASS\x1b[0m — all phases completed successfully.");
    } else {
...
fn print_corim_payload(file_name: &str, corim_map: &corim_rs::CorimMap, env_colors: &HashMap<String, &str>) {
            ...
            corim_rs::ConciseTagTypeChoice::Ev(tagged_coev) => {
                ...
                if let Some(ref triples) = coev.ev_triples.evidence_triples {
                    ...
                    for (j, triple) in triples.iter().enumerate() {
                        ...
                        let color = env_colors.get(&env_label(&triple.ref_env)).copied().unwrap_or("");
...
fn print_environment(env: &corim_rs::EnvironmentMap, color: &str) {
        ...
        if let Some(ref class_id) = class.class_id {
            if let Some(bytes) = class_id.as_bytes() {
                match std::str::from_utf8(bytes) {
                    ...
                    Err(_) => println!("                {}Class ID:  {:?}{}", color, class_id, C_RESET),
                }
            } else {
                println!("                {}Class ID:  {:?}{}", color, class_id, C_RESET);
            }
        }
        if let Some(ref vendor) = class.vendor {
    ...
    if let Some(ref instance) = env.instance {
        println!("                {}Instance:  {:?}{}", color, instance, C_RESET);
    }
