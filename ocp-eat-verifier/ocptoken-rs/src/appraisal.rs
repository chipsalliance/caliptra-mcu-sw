...
pub fn appraise(
    ...
    // 1. Collect reference triples from all CoRIM tags.
...
fn format_mkey(meas: &MeasurementMap) -> String {
    match &meas.mkey {
        ...
        Some(corim_rs::MeasuredElementTypeChoice::UInt(n)) => format!("mkey={}", n),
    ...
    if let Some(ref ref_digests) = ref_mval.digests {
        if let Some(ref ev_digests) = ev_mval.digests {
            for rd in ref_digests {
                if let Some(ed) = ev_digests.iter().find(|ed| ed.alg == rd.alg && ed.val == rd.val) {
            ...
            Some(ev_digests) => {
                for rd in ref_digests {
                    ...
                    match found {
                        ...
                        None => {
                            mismatches.push(format!("digest({:?}): not present in evidence", rd.alg));
                        }
                        _ => {} // matching
                    }
                }
