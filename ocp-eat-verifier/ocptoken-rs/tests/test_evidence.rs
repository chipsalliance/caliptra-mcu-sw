...
fn decode_measurement_block_fd_bin() {
    ...
    // Read the bin generated by mctp spdm validator
    // TODO change it to a cli app t
    let encoded = fs::read(
        "/home/mei_lu/caliptra-mcu-sw/spdm-mctp-validator-test-results/measurement_block_fd.bin",
    )
    .expect("failed to read measurement_block_fd.bin");

    println!("encoded len: {}", encoded.len());

    let _evidence = match Evidence::decode(&encoded[3..]) {
        Ok(ev) => ev,
        Err(e) => {
            eprintln!("Evidence::decode failed");
            eprintln!("Error: {:#?}", e);
            eprintln!("Input length: {}", encoded.len());
            eprintln!(
                "First 32 bytes (hex): {}",
                hex::encode(&encoded[..32.min(encoded.len())])
            );
            panic!("Evidence::decode failed");
        }
    };
    // Basic sanity check
    //assert!(evidence.signed_eat.is_some());
}*/
