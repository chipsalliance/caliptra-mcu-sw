...
| MC_EXPORT_ATTESTED_CSR            | 0x4D45_4143 ("MEAC") | Exports an attested DER-encoded PKCS#10 CSR for a specified device key.                           |
...
Exports an attested Certificate Signing Request (CSR) for a specified device key. The response payload is the DER-encoded PKCS#10 CSR returned by firmware. A 32-byte nonce is included in the CSR request information for freshness.
...
| csr_data    | u8[data_len]   | Attested CSR payload (DER-encoded PKCS#10 CSR).           |
