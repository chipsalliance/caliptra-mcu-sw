...
If the MCU ROM is built with the `stable-owner-key` feature, provision the
32-byte `stable_owner_key_personalization_seed` field and at least one valid
HEK seed slot. In the reference fuse map, the personalization seed is mapped to
`CPTRA_SS_VENDOR_SPECIFIC_NON_SECRET_FUSE_6`, but integrators may choose a
different location by updating `hw/fuses.hjson` and regenerating the generated
fuse definitions. During cold boot, the MCU ROM uses the personalization seed
as the `CM_DERIVE_STABLE_KEY(OwnerKey)` input and forwards the selected HEK
seed to Caliptra for the derivation. The `stable-owner-key` and `ocp-lock` ROM
features consume the HEK seed path in different ways and must not be enabled in
the same ROM image.
