...
HEK Seed states are defined by the EKP spec. The MCU runtime uses this spec for attestation and state management. KMB uses a simplified state for communicating to KMB. The simplified states are defined in the OCP LOCK specification and are compatible with the expanded states defined in EKP.
...
The OCP LOCK state is a critical part of the system's security posture. To ensure consistency, the ROM **always** writes the OCP LOCK state into the handoff table, even if OCP LOCK is not explicitly enabled or no valid HEK is found.

- **`HekState`**: Contains `active_state`, `active_slot`, and `total_slots`.
- **Initialization**: Managed via the `HandoffArgs` struct passed to `HandoffData::new()`.
