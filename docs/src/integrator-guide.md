...
ROM. The map binds Caliptra SoC manifest `component_id` values to
`SOC_IMAGE_MIN_SVN[i]` fuse slots. Multiple component IDs may share one slot if
those components always move together and use the same floor policy.
...
MCU ROM does stash ROM-owned measurements that only it can observe. When
`RomParameters::stash_rom_digest` is set, ROM stashes its own ROM digest. ROM
