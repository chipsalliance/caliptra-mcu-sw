...
Implementation note: the current `spdm-lite` SET_CERTIFICATE flash-store path is intentionally test-only unless an integrator supplies a production authorization/key-binding policy. The emulator user app advertises this command only when built with `test-mctp-spdm-set-certificate`, which explicitly sets `allow_unauthenticated_set_certificate: true`. Default builds must not advertise SET_CERTIFICATE until secure-session/authorization and key-pair binding validation are enabled.
