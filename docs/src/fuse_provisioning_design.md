# Caliptra MCU Fuse Provisioning

## Proposal Overview

Fuse provisioning for Caliptra MCU, occurs across two distinct manufacturing
...
1. **`Test_Unlocked` State**: Programming initial fuses, primarily the Lifecycle
    transition tokens required to advance the state machine.
2. **`Manuf` State**: Programming the remaining production fuses, such as vendor
    public key hashes and revocation fields.

To provision these fuses, we are proposing creating dedicated bare-metal
binaries to burn the fuses, which will be loaded into SRAM over JTAG by the
tester at each respective stage.
...
The plan is to build two main software components that work together with the
factory tester to program the chip, one of which is already completed.

```mermaid
graph TD
    subgraph Offline Phase [Host / Build System]
        A["Fuse Values (hjson)"] -->|Read by| B["Fuse Library (build.rs)"]
        B -->|Generates| D["Generated Rust Array"]
        D -->|Compiled into| F["Bare-metal Binary"]
        G["runtime/bare-metal"] -->|Template for| F
    end

    subgraph Online Phase [Manufacturing / Tester]
        H["Factory Tester"] -->|1. Loads via JTAG| F
        F -->|2. Runs in SRAM| I["Caliptra MCU"]
        I -->|3. Writes to| J["Fuse Controller"]
        J -->|4. Blows| K["OTP Fuses"]
    end
```
...
In order to tell our bare-metal code which fuse values to burn, we've written a
    ...
    * There currently exists a base implementation of bare-metal (non-Tock)
      binaries, but some improvements are also proposed:
        ...
        * **Code-Level Configuration Generation**: Have the `firmware-bundler`
          ...
          "magic numbers" in the bare-metal source code and ensures portability
          across simulation and hardware.

### JTAG Side Loading

The provisioning process relies on loading the bare-metal binary into the MCU's
SRAM and controlling its execution over JTAG. 
...
* **Execution Control**: The tester halts the CPU and updates the Debug Program
    Counter (**`dpc`** CSR) to point to the start address of the loaded binary
    in SRAM before resuming execution.
* **Completion Signaling**: The tester configures the Debug Control and Status
    ...
    executing an `ebreak` instruction, which halts the CPU and yields control
    back to the tester.
