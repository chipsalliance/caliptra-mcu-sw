...
impl Emulator {
    ...
    pub fn from_args_with_callbacks(
        ...
        let primary_flash_initial_content = if let Some(flash_image_path) = &cli.primary_flash_image
        {
        ...
        let secondary_flash_initial_content =
            if let Some(flash_image_path) = &cli.secondary_flash_image {
                const FLASH_SIZE: usize =
                    DummyFlashCtrl::PAGE_SIZE * DummyFlashCtrl::MAX_PAGES as usize;
                let mut flash_image = vec![0; FLASH_SIZE];
                let mut file = File::open(flash_image_path)?;
        ...
        if let Some(pldm_fw_pkg_path) = &cli.streaming_boot {
