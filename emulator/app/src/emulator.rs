    ...
    pub fn from_args_with_callbacks(
            ...
            .and_then(|p| Otp::read_lifecycle_from_file(p))
        {
            let mem: [u8; caliptra_mcu_otp_lifecycle::LIFECYCLE_MEM_SIZE] =
                lc_bytes.try_into().map_err(|v: Vec<u8>| {
                    io::Error::new(
                        io::ErrorKind::InvalidData,
                        format!(
                            "OTP lifecycle partition has wrong size: got {} bytes, expected {}",
                            v.len(),
                            caliptra_mcu_otp_lifecycle::LIFECYCLE_MEM_SIZE
                        ),
                    )
                })?;
            let (state, count) = caliptra_mcu_otp_lifecycle::lc_decode_memory(&mem).map_err(
                |e| {
                    // Dump the raw LIFE_CYCLE partition bytes (before reversal) plus
                    // which sub-decode failed, so an opaque "DecodeFailed" is actionable.
                    let mut reversed = mem;
                    reversed.reverse();
                    let state_slice = &reversed[..caliptra_mcu_otp_lifecycle::LIFECYCLE_STATE_SIZE];
                    let count_slice = &reversed[caliptra_mcu_otp_lifecycle::LIFECYCLE_STATE_SIZE..];
                    let state_mem: [u8; caliptra_mcu_otp_lifecycle::LIFECYCLE_STATE_SIZE] =
                        state_slice.try_into().expect("len checked");
                    let count_mem: [u8; caliptra_mcu_otp_lifecycle::LIFECYCLE_COUNT_SIZE] =
                        count_slice.try_into().expect("len checked");
                    let state_dec =
                        caliptra_mcu_otp_lifecycle::lc_decode_state_mem(&state_mem);
                    let count_dec =
                        caliptra_mcu_otp_lifecycle::lc_decode_count_mem(&count_mem);
                    ...
                    let stage = match (state_dec, count_dec) {
                        (Err(_), Err(_)) => "both state and count tables miss",
                        (Err(_), Ok(_)) => "state table miss (count OK)",
                        (Ok(_), Err(_)) => "count table miss (state OK)",
                        (Ok(_), Ok(_)) => "unknown (both sub-decodes succeeded)",
                    };
                    io::Error::new(
                        io::ErrorKind::InvalidData,
                        format!(
                            "Failed to decode lifecycle from OTP fuses: {e}\n\
                             \tStage: {stage}\n\
                             \tsub-decode results: state={:?} count={:?}\n\
                             \tLIFE_CYCLE partition raw bytes ({} B, as read from file):\n\t  {}\n\
                             \tReversed state region ({} B, what lc_decode_state_mem saw):\n\t  {}\n\
                             \tReversed count region ({} B, what lc_decode_count_mem saw):\n\t  {}\n\
                             \tHint: bytes must match the Caliptra RTL ECC tables in lc_ctrl_state_pkg.sv. \
                             If this OTP image was generated by an OpenTitan-style tool with different ECC \
                             constants, regenerate it with caliptra_mcu_otp_lifecycle::lc_generate_memory.",
                            state_dec,
                            count_dec,
                            mem.len(),
                            hex(&mem),
