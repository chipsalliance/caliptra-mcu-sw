...
use emulator_periph::{MCU_MAILBOX0_SRAM_SIZE, McuMailbox0Internal};
use registers_generated::mci::bits::MboxExecute;
use tock_registers::interfaces::Readable;

#[derive(Clone)]
pub struct McuMailboxTransport {
    mbox: McuMailbox0Internal,
}

impl McuMailboxTransport {
    pub fn new(mbox: McuMailbox0Internal) -> Self {
    ...
    pub fn execute(&self, cmd: u32, payload: &[u8]) -> Result<(), McuMailboxError> {
        ...
        // Sender writes data length in words to MBOX_DLEN
        self.mbox
            .regs
            .lock()
            .unwrap()
    ...
    pub fn get_execute_response(&self) -> Result<McuMailboxResponse, McuMailboxError> {
        ...
        let dlen = self
            ...
            / 4;
        println!("Emulator: Received dlen {}", dlen);
