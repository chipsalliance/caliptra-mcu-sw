...
use caliptra_emu_bus::{ActionHandle, Clock, Device, Event, EventData, ReadWriteRegister, Timer, TimerAction};
...
impl Mci {
    ...
    pub fn new(clock: &Clock, ext_mci_regs: caliptra_emu_periph::mci::Mci, irq: Rc<RefCell<Irq>>) -> Self {
...
impl MciPeripheral for Mci {
    ...
    > {
        caliptra_emu_bus::ReadWriteRegister::new(self.ext_mci_regs.regs.borrow().reset_reason)
    }
    ...
    fn poll(&mut self) {
        ...
        if self.timer.fired(&mut self.op_mcu_assert_mcu_reset_status_action) {
            // MCU is now in reset, assert the reset status
            println!("[MCI] TimerAction::Set Reset Status");
        ...
        if self.timer.fired(&mut self.op_mcu_deassert_mcu_reset_status_action) {
            // MCU is now out of reset, deassert the reset status
            println!("[MCI] TimerAction::Clear Reset Status");
            self.ext_mci_regs.regs.borrow_mut().reset_status &= !RESET_STATUS_MCU_RESET_MASK;
            self.op_mcu_deassert_mcu_reset_status_action = None;

            println!(
                "Deasserting Interrupt"
            );

            self.irq.borrow_mut().set_level(false);
            println!(
                "Deasserted Interrupt"
            );                  
        }
