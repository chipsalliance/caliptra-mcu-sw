...
use crate::mcu_mbox0::MciMailboxInternal;
...
pub struct Mci {
    ...
    mcu_mailbox0: Option<MciMailboxInternal>, // Integrate MCU mailbox
}
...
impl Mci {
    pub fn new(
        ...
        ext_mci_regs: caliptra_emu_periph::mci::Mci,
        irq: Rc<RefCell<Irq>>,
        mcu_mailbox0: Option<MciMailboxInternal>,
    ) -> Self {
        ...
        //let mcu_mailbox = crate::mcu_mbox0::MciMailboxInternal::new(clock);
        Self {
            ext_mci_regs,
...
impl MciPeripheral for Mci {
    ...
    fn read_mcu_mbox0_csr_mbox_sram(&mut self, index: usize) -> caliptra_emu_types::RvData {
        self.mcu_mailbox0
            .as_mut()
            .expect("MCU mailbox not initialized")
            .regs
            .borrow_mut()
    ...
    fn poll(&mut self) {
        ...
        // XS: if mcu mailbox received IRQ, process it
        if let Some(event) = self.mcu_mailbox0.as_mut().and_then(|mb| mb.get_notif_irq()) {
            println!("[xs debug] MCI: Processing mailbox event: {:?}", event);
            //use registers_generated::mci::bits::{Notif0IntrEnT, Notif0IntrT};
            ...
            println!(
                "[xs debug] MCI: Notif0IntrEnR: {:#x}, Notif0InternalIntrR: {:#x}",
                notif_en, notif_reg
            );
            // Set the correct bit for the event if enabled
            match event {
                crate::mcu_mbox0::InterruptToMcu::Mbox0CmdAvailable => {
                    if notif_en & Notif0IntrEnT::NotifMbox0CmdAvailEn::SET.value != 0 {
                        notif_reg |= Notif0IntrT::NotifMbox0CmdAvailSts::SET.value;
                    }
                }
                crate::mcu_mbox0::InterruptToMcu::Mbox0TargetDone => {
            ...
            println!(
                "[xs debug] MCI: Notif0InternalIntrR updated to: {:#x}",
                notif_reg
            );
        }
    }
...
mod tests {
    use super::*;
    use crate::mcu_mbox0::InterruptToMcu;
    ...
    fn check_mcu_mailbox0_interrupt(
        clock: &Clock,
        mci_bus: &mut MciBus,
        mcu_mailbox: &mut MciMailboxInternal,
        irq_event: InterruptToMcu,
        en_bit: u32,
        sts_bit: u32,
    ) {
    ...
    fn test_mailbox_interrupt_handling() {
        ...
        check_mcu_mailbox0_interrupt(
            ...
            &mut mci_bus,
            &mut mcu_mailbox,
            InterruptToMcu::Mbox0CmdAvailable,
        ...
        check_mcu_mailbox0_interrupt(
            ...
            &mut mci_bus,
            &mut mcu_mailbox,
            InterruptToMcu::Mbox0TargetDone,
