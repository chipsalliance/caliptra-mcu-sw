...
impl DummyDoeMbox {
    ...
    #[allow(dead_code)]
    pub fn new(clock: &Clock, event_irq: Irq) -> Self {
        DummyDoeMbox {
            mbox_sram: Rc::new(RefCell::new(Vec::new())),
    ...
    fn reset(&mut self) {
        ...
        self.mbox_sram.borrow_mut().clear();
    }
...
impl DoeMboxPeripheral for DummyDoeMbox {
    ...
    > {
        caliptra_emu_bus::ReadWriteRegister::new(self.mbox_status.reg.get())
    }
    ...
    fn write_doe_mbox_status(
        ...
        self.mbox_status.reg.set(val.reg.get());
    }
    ...
    > {
        caliptra_emu_bus::ReadWriteRegister::new(self.mbox_event.reg.get())
    }
    ...
    fn read_doe_mbox_sram(&mut self, index: usize) -> caliptra_emu_types::RvData {
        ...
        let sram = self.mbox_sram.borrow();
    ...
    fn write_doe_mbox_sram(&mut self, val: caliptra_emu_types::RvData, index: usize) {
        ...
        let mut sram = self.mbox_sram.borrow_mut();
        sram.push(val);
    }
}
...
mod tests {
    ...
    fn test_helper_setup_autobus(clock: &Clock) -> AutoRootBus {
        ...
        let doe_mbox = Box::new(DummyDoeMbox::new(clock, doe_event_irq));
