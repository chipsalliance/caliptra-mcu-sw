...
Abstract:
    ...
    This peripheral provides a simple interface for sending and receiving Ethernet
    frames to/from a TAP device, enabling network boot recovery functionality.

    This implementation uses the auto-generated EthernetPeripheral trait from
    registers/generated-emulator for register access patterns.
...
/// Maximum Ethernet frame size (including headers, excluding FCS)
...
/// Maximum number of frames that can be queued for reception
const RX_QUEUE_DEPTH: usize = 16;

/// Trait for TAP device backends
...
impl LinuxTapDevice {
    ...
    pub fn open(name: &str) -> std::io::Result<Self> {
        ...
        let ret = unsafe { libc::ioctl(fd, 0x400454ca /* TUNSETIFF */, &ifr as *const _) };
...
impl Ethernet {
    ...
    fn transmit(&mut self) {
        ...
        if let Some(tap) = &self.tap {
            if let Ok(mut tap) = tap.lock() {
                match tap.send(&self.tx_buffer[..frame_len]) {
                    Ok(sent) => {
                        ...
                        let hex: Vec<String> = self.tx_buffer[..debug_len].iter().map(|b| format!("{:02x}", b)).collect();
            ...
            eprintln!("[ETH TX] No TAP device configured, frame size: {}", frame_len);
        }
    ...
    /// Poll for incoming frames from the TAP device
    fn poll_rx(&mut self) {
                        ...
                        if len >= 14 {
                            ...
                            if ethertype == 0x0800 && len > 42 {
                                eprintln!("[ETH RX] *** IPv4 packet! Protocol: {}, Src Port: {}, Dst Port: {}",
                                    ...
                                    ((buffer[36] as u16) << 8) | buffer[37] as u16); // UDP dst port
                            }
                        }
                        buffer.truncate(len);
    ...
    /// Get the current RX frame (if any)
    ...
    fn pop_rx_frame(&mut self) {
        if let Some(frame) = self.rx_queue.front() {
            if frame.len() >= 14 {
                ...
                println!("[ETH POP] Popping frame, len={}, EtherType: {:04x}, queue_len_after={}", 
                    frame.len(), ethertype, self.rx_queue.len().saturating_sub(1));
...
mod tests {
    ...
    fn test_mac_address() {
        ...
        // Write MAC address
        eth_bus.write(RvSize::Word, REG_MAC_LOW, 0x04030201).unwrap();
    ...
    fn test_tx_buffer_write() {
        ...
        eth_bus.write(RvSize::Word, REG_TX_DATA_BASE, 0xDEADBEEF).unwrap();
        eth_bus
        ...
        eth_bus.write(RvSize::Word, REG_CTRL, CTRL_TX_START).unwrap();
