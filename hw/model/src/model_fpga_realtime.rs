...
pub struct ModelFpgaRealtime {
    ...
    i3c_next_private_read_len: VecDeque<u16>,
        ...
        while !self.i3c_next_private_read_len.is_empty() {
            ...
            match self.base.i3c_controller().read(private_read_len) {
                ...
                Err(e) => {
                    writeln!(
                        eoutput(),
                        "Error receiving I3C private read: {:?}; will retry",
                        e
                    )
                    .unwrap();
                    // retry
                    self.i3c_next_private_read_len.push_front(private_read_len);
                }
            }
        }
    }
    ...
    fn handle_ocp_lock_key_release(&mut self) {
        if !self.printed_ocp_lock_key_release {
                ...
                .any(|r| r.get() != 0)
            {
                ...
                // unclear why, but keeping this function makes the system boot faster
                writeln!(
                    side_output(),
...
impl McuHwModel for ModelFpgaRealtime {
        ...
        Self: Sized,
    {
        ...
        let m = Self {
            ...
            i3c_handle,
            i3c_tx,
            i3c_next_private_read_len: VecDeque::new(),
