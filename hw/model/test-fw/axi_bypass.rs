...
use registers_generated::i3c::bits::{
    DeviceStatus0::DevStatus, IndirectFifoCtrl0, IndirectFifoStatus0::{Empty, Full}, RecIntfCfg::{self}, RecoveryCtrl::ActivateRecImg, RecoveryStatus::DevRecStatus
};
...
fn activate_recovery_image(env: &mut RomEnv) {
    // TODO(clundin): The generated register code seems to have the wrong offset
    // for this register.
    //
    // Ideally the code should look like this, but doesn't work currently.
    // Regenerating the registers should solve this.
    //
    // ```rust
    // env.i3c_base
    //     .sec_fw_recovery_if_recovery_ctrl
    //     .modify(ActivateRecImg.val(0xF));
    // ```
    //
    // Use a hard coded constant for now.
    env.i3c_base
        .sec_fw_recovery_if_recovery_ctrl
        .set(0xF00);
}
