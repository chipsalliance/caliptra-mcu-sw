...
pub trait Mailbox<'a> {
    /// Sends a command and associated data to the mailbox (Sender mode).
    ///
    /// # Arguments
    ///
    ...
    fn send_request(&self, command: u32, request_data: &[u8]) -> Result<(), ErrorCode>;
...
pub trait MailboxClient<'a> {
    ...
    fn request_received(&self, command: u32, rx_buf: &'a mut [u8], length: usize);
    ...
    /// * `length` - Number of valid bytes in `rx_buf`.
    fn response_received(&self, status: MailboxStatus, rx_buf: &'a mut [u8], length: usize);
