...
/// Builds a standard userspace flash partition over emulator-owned memory.
///
/// Keeping the normal `FlashPartition` syscall interface means Runtime DOT code
/// uses the same `SpiFlash` API on emulator and FPGA; only the kernel backing
