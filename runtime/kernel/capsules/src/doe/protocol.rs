...
pub const DOE_DATA_OBJECT_HEADER_LEN_DW: u32 = 2; // 8 bytes, 2 dwords
pub const DOE_DISCOVERY_REQ_RESP_LEN_DW: u32 = 1; // 4 bytes, 1 dword
const PCISIG_DOE_VENDOR_ID: u16 = 0x0001;
const LENGTH_FIELD_BITS: u32 = 18;
const MAX_DOE_DATA_OBJECT_LEN_DW: u32 = 1 << (LENGTH_FIELD_BITS - 1); // 262144 dwords, 1048576 bytes
pub const DOE_DISCOVERY_DATA_OBJECT_LEN_DW: u32 =
    DOE_DATA_OBJECT_HEADER_LEN_DW + DOE_DISCOVERY_REQ_RESP_LEN_DW; // 12 bytes, 3 dwords
const LENGTH_MASK: u32 = (1 << (LENGTH_FIELD_BITS - 1)) - 1;
...
impl DoeDataObjectHeader {
    ...
    pub fn decode(data: &[u32]) -> Option<Self> {
        ...
        if data.len() < DOE_DATA_OBJECT_HEADER_LEN_DW as usize {
            return None;
        }
        ...
        let length = (word1 & LENGTH_MASK) as u32;
    ...
    pub fn validate(&self, data_obj_len: u32) -> bool {
        ...
        match self.data_object_type() {
            DataObjectType::Unsupported => return false,
            _ => {}
        }
        ...
        if self.data_object_type() == DataObjectType::DoeDiscovery {
            ...
            if self.length != DOE_DISCOVERY_REQ_RESP_LEN_DW || self.length != data_obj_len {
                return false;
            }
        } else {
            // For other types, length must be at least DOE_DISCOVERY_DATA_OBJECT_LEN_DW and match data_obj_len
            if self.length < DOE_DISCOVERY_DATA_OBJECT_LEN_DW || self.length != data_obj_len {
                return false;
            }
        }
