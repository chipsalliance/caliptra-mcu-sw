...
//! # Ownership model
//!
//! The *type* is shared across tasks; an *instance* is not. Each task
//! declares its own pool — typically via [`StaticBitmapAllocatorCell`] over a
//! task-local static buffer — and that pool has exactly one owning task.
//!
//! # Soundness
//!
//! [`BitmapAllocator`] is `!Send` + `!Sync`; the bitmap is mutated through
//! [`UnsafeCell`] and assumes a single-threaded owner. Sharing one instance
//! between tasks, or calling its methods concurrently, is undefined behavior.
...
// free slots, marks them used, moves `v` in, and returns a `ScratchBox<T>`
...
impl BitmapAllocator {
    ...
    /// * The memory must outlive every [`ScratchBox`] / [`BitmapBytes`]
    ...
    ///
    /// A correctly-functioning steady-state consumer returns to a known
    /// baseline (zero, or the count of slots held by persistent allocations
    /// owned by long-lived caller state) between requests. A drift above the
    /// baseline points at a leaked [`BitmapBytes`] / [`ScratchBox`] handle.
    ...
    /// # Safety
    ///
    /// All outstanding [`ScratchBox`] / [`BitmapBytes`] handles created
    ...
    /// # Returns
    ///
    /// * `Ok(ScratchBox<T>)` — RAII owner; drops the value in place and
    ...
    pub fn alloc<T>(&self, value: T) -> McuResult<ScratchBox<'_, T>> {
...
#[must_use = "dropping a ScratchBox without holding it would immediately free the underlying slots"]
pub struct ScratchBox<'a, T> {
...
impl<T> core::ops::Deref for ScratchBox<'_, T> {
    type Target = T;
...
impl<T> core::ops::DerefMut for ScratchBox<'_, T> {
...
///
/// Like [`ScratchBox<[u8; _]>`] but lets the holder shrink the logical
...
mod tests {
    ...
    ///
    /// This test gates removal of the static large buffer. If first-fit
    ...
    fn fragmentation_soak_8k_alloc_after_realistic_workload() {
        ...
        // allocation. This is the GATE: if it fails, we need directional
            ...
            Err(_) => {
                panic!(
                    "FRAGMENTATION SOAK FAILED: 8 KiB allocation failed after 50 cycles. \
                     ...
                     Deleting the static large buffer needs directional allocation.",
