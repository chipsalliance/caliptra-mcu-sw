...
async fn dpe_sign_mldsa87_message<A: ApiAlloc>(
    ...
    let mut public_key = alloc.alloc(CERTIFY_KEY_MLDSA87_PUBKEY_SIZE)?;
    let public_key: &mut [u8; CERTIFY_KEY_MLDSA87_PUBKEY_SIZE] =
        public_key.as_mut().try_into().map_err(|_| INTERNAL_BUG)?;
