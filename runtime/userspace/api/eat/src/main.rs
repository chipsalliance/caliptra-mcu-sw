...
#[cfg(feature = "crypto")]
use eat_encoder::{EatEncoder, DebugStatus, MeasurementFormat, OcpEatClaims, ProtectedHeader, CoseHeaderPair, cose_headers, ConciseEvidence};
...
impl DeviceKey {
    ...
    fn generate_mock_cert_chain(public_key: &PublicKey) -> Vec<u8> {
        ...
        let mut cert = vec![
            ...
            // Serial Number (20 bytes - from real cert)
            0x02, 0x14, 0x40, 0xea, 0x74, 0x7c, 0x71, 0x80, 0xf9, 0xc9, 
            0x31, 0xdd, 0xe7, 0x94, 0x6c, 0x15, 0x17, 0xde, 0x0e, 0x14, 0xcb, 0x3e,
            
            // Signature Algorithm Identifier
            0x30, 0x0a,
            0x06, 0x08, 0x2a, 0x86, 0x48, 0xce, 0x3d, 0x04, 0x03, 0x02, // ecdsa-with-SHA256 (changed from SHA384 to match OpenSSL)
            ...
            0x13, 0x02, 0x55, 0x53, // C=US
            0x31, 0x15,
            0x30, 0x13,
            0x06, 0x03, 0x55, 0x04, 0x0a,
            0x0c, 0x0c, 0x45, 0x78, 0x61, 0x6d, 0x70, 0x6c, 0x65, 0x20, 0x43, 0x6f, 0x72, 0x70, // O=Example Corp
            0x31, 0x17,
            0x30, 0x15,
            0x06, 0x03, 0x55, 0x04, 0x03,
            0x0c, 0x0e, 0x44, 0x65, 0x76, 0x69, 0x63, 0x65, 0x20, 0x41, 0x74, 0x74, 0x20, 0x4b, 0x65, 0x79, // CN=Device Att Key
            
            // Validity (using current time format from OpenSSL)
            0x30, 0x1e,
            0x17, 0x0d, // UTCTime
            0x32, 0x35, 0x30, 0x39, 0x31, 0x38, 0x30, 0x37, 0x30, 0x36, 0x30, 0x35, 0x5a, // 250918070605Z (valid from)
            0x17, 0x0d, // UTCTime  
            0x32, 0x36, 0x30, 0x39, 0x31, 0x38, 0x30, 0x37, 0x30, 0x36, 0x30, 0x35, 0x5a, // 260918070605Z (valid to)
        ...
        cert.extend_from_slice(&[
            ...
            0x30, 0x1d,
            0x06, 0x03, 0x55, 0x1d, 0x0e, // subjectKeyIdentifier OID
            0x04, 0x16, 0x04, 0x14, // OCTET STRING (20 bytes)
            0x3a, 0x5e, 0xb7, 0x51, 0x53, 0x4f, 0x2c, 0x25, 0x9e, 0x04,
            0x85, 0xad, 0xf1, 0x8e, 0xd7, 0xe2, 0xd7, 0x10, 0xac, 0x7e,
            
            // Authority Key Identifier extension
            0x30, 0x1f,
            0x06, 0x03, 0x55, 0x1d, 0x23, // authorityKeyIdentifier OID
            ...
            0x30, 0x0f,
            0x06, 0x03, 0x55, 0x1d, 0x13, // basicConstraints OID
        ...
        cert.extend_from_slice(&[
            ...
            0x85, 0xf2, 0x71, 0xa4, 0x12, 0x27, 0xd2, 0xe3, 0x4f, 0x80, 0x6d, 0xeb,
            0xd4, 0x41, 0x78, 0x3b, 0x5c, 0x2f, 0x3e, 0x4a, 0x58, 0x92, 0x5a, 0x7d,
            0xd1, 0x2c, 0x8f, 0x91, 0x43, 0xb2, 0xe9, 0xfa, 0x6c, 0x88, 0xa1, 0x7b,
            0x25, 0x4e, 0xd8, 0x9b, 0x73, 0x11, 0x45, 0x22, 0x8f, 0x6a, 0x33, 0x44,
            0x02, 0x31, 0x00, // INTEGER s (49 bytes with leading zero)
            0xa7, 0x89, 0x4c, 0x23, 0xe1, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x11,
...
fn decode_cbor_file(cbor_file_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !output.stderr.is_empty() {
        eprintln!("Decoder stderr: {}", String::from_utf8_lossy(&output.stderr));
    }
    
    if !output.status.success() {
        return Err(format!("Python decoder failed with exit code: {:?}", output.status.code()).into());
    }
...
pub fn create_example_eat(
    ...
    let protected_len = {
        ...
        encoder.encode_protected_header(&protected_header)
    ...
    let payload_len = {
        ...
        encoder.encode_ocp_eat_claims(&claims)
            .map_err(|_| EatError::EncodingError)?;
        encoder.len()
    };
    ...
    // Generate signature
    let signature = device_key.sign(&sig_context_buffer[..sig_context_len])
        .map_err(|_| EatError::EncodingError)?;
...
fn create_mock_concise_evidence_structured() -> eat_encoder::ConciseEvidence<'static> {
    use eat_encoder::{ClassMap, EnvironmentMap, MeasurementMap, MeasurementValue, DigestEntry, ConciseEvidenceMap, EvTriplesMap, EvidenceTripleRecord};
    ...
    static DIGEST_DATA: &[u8; 48] = &[
        ...
        0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18,
        0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x56,
    ];
    ...
    static FIRMWARE_MEASUREMENT: [MeasurementMap<'static>; 1] = [
        MeasurementMap {
            key: 0,
    ...
    static HW_CONFIG_MEASUREMENT: [MeasurementMap<'static>; 1] = [
        MeasurementMap {
            key: 0,
            mval: MeasurementValue {
                version: None,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output_file = if args.len() > 1 {
        ...
        println!("  <output_file.cbor>  Filename for the generated CBOR token (saved in output/ folder)");
        println!();
        println!("Decode mode:");
        println!("  Decode an existing CBOR token using the Python decoder");
        ...
        println!();
        println!("Examples:");
        println!("  {} example_eat_token.cbor          # Generate output/example_eat_token.cbor", args[0]);
        println!("  {} --decode output/my_token.cbor   # Decode existing token", args[0]);
        return Ok(());
    };
    ...
    println!("  Public key: {:02x?}", &device_key.public_key.to_encoded_point(false).as_bytes()[1..9]);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!();
    println!("Examples:");
    println!("  cargo run --features std,crypto -- my_eat_token.cbor  # Creates output/my_eat_token.cbor");
    ...
    if args.len() > 1 {
        ...
        println!("The output file parameter '{}' will be ignored in this mode.", args[1]);
        println!("Files would be saved to output/{} when crypto features are enabled.", args[1]);
    }
...
fn print_cbor_diagnostic(data: &[u8]) {
    ...
    while pos < data.len() && pos < 32 { // Limit analysis to first 32 bytes
        let byte = data[pos];
        ...
        match major_type {
            ...
            4 => {
                println!("{}Array (length: {})", indent, 
                    if additional_info <= 23 { additional_info as usize } else { 0 });
                depth += 1;
                pos += 1;
            }
            5 => {
                println!("{}Map (pairs: {})", indent,
                            if additional_info <= 23 { additional_info as usize } else { 0 });
                depth += 1;
                pos += 1;
            }
            ...
            7 => {
                match additional_info {
                    20 => { println!("{}False", indent); pos += 1; }
                    21 => { println!("{}True", indent); pos += 1; }
                    22 => { println!("{}Null", indent); pos += 1; }
                    23 => { println!("{}Undefined", indent); pos += 1; }
                    31 => { 
