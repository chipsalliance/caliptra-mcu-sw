...
def analyze_cose_signature(signature):
    ...
    logger.debug(f"Signature ({len(signature)} bytes): {signature.hex()}")

    # Try to parse as DER-encoded ECDSA signature
    try:
        if len(signature) > 6 and signature[0] == 0x30:  # DER SEQUENCE
            sig_len = signature[1]
            logger.debug(f"DER-encoded ECDSA signature (declared length: {sig_len})")
            
            # Parse r component
            if signature[2] == 0x02:  # INTEGER
                r_len = signature[3]
                r_value = signature[4:4+r_len]
                logger.debug(f"  r component ({r_len} bytes): {r_value.hex()}")
                ...
                if s_offset < len(signature) and signature[s_offset] == 0x02:  # INTEGER
                    ...
                    logger.debug(f"  s component ({s_len} bytes): {s_value.hex()}")
                                           
    except Exception as e:
        logger.error(f"Error analyzing signature structure: {e}")

def analyze_certificate_headers(key_value, value_data):
    """Analyze certificate in unprotected headers"""
    if key_value == 33:  # X5CHAIN
        logger.debug(f"  Key {key_value} (X5CHAIN): Certificate chain ({len(value_data)} bytes)")
        logger.debug(f"    Certificate DER (first 32 bytes): {value_data[:32].hex()}")

        # Try to identify certificate type
        if value_data[:2] == b'\x30\x82':
            import struct
            cert_len = struct.unpack('>H', value_data[2:4])[0]
            logger.debug(f"    X.509 certificate (declared length: {cert_len + 4} bytes)")
            
            # Look for common certificate fields
            cert_str = value_data.hex()
            if '2a8648ce3d020106' in cert_str:  # ecPublicKey OID
                logger.debug(f"    Algorithm: ECDSA (Elliptic Curve Public Key)")
            if '2b8104002200' in cert_str:  # secp384r1 OID
                logger.debug(f"    Curve: secp384r1 (P-384)")
            if '2a8648ce3d040303' in cert_str:  # ecdsaWithSHA384 OID
                logger.debug(f"    Signature Algorithm: ECDSA with SHA-384")
