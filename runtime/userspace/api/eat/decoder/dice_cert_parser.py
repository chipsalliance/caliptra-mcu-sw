...
in a human-readable format, including support for TCG DICE extensions commonly
used in SPDM certificate chains.
"""
...
# Common hash algorithm OIDs for readable display
_HASH_ALGORITHM_OIDS = {
...
def parse_tcg_dice_multi_tcbinfo(raw_data: bytes) -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        ...
        for tcb_info in multi_tcb_info:
        ...
        logger.debug(f"Failed to parse MultiTcbInfo with schema: {e}")
        return None
...
def _parse_single_tcbinfo(tcb_info: TcbInfo) -> Optional[Dict[str, Any]]:
    ...
    if tcb_info['fwids'] is not None:
        ...
        try:
            for i, fwid in enumerate(tcb_info['fwids']):
                ...
                hash_alg_name = _HASH_ALGORITHM_OIDS.get(hash_alg_oid, hash_alg_oid)
                fwid_info = {
                    'hash_alg': hash_alg_name,
    ...
    if tcb_info['vendor_info'] is not None and tcb_info['vendor_info'].contents:
        result['vendor_info'] = f"0x{tcb_info['vendor_info'].contents.hex()}"
