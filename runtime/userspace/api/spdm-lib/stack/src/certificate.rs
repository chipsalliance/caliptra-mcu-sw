...
pub(crate) async fn handle_get_certificate_req<'a, Pal: SpdmPal>(
    ...
    // Two independent caps apply. The chain format's Length field is
    // 16-bit before 1.4 and 32-bit from 1.4, and the request form's
    // offset/length fields are 16-bit for a standard GET_CERTIFICATE
    // and 32-bit for LargeCertChain. A 1.4 requester using the
    // standard form therefore still can't address past 64 KiB, so take
    // the tighter of the two.
    let max_chain_len = max_cert_chain_len(state.version).min(req.max_length_cap());
    if total_len_usize > max_chain_len {
        // A 1.4 requester can always retry with LargeCertChain, so tell
        // it the actual size via CertChainTooLarge. Below 1.4 (or when
        // the chain overflows even the 32-bit field) there is no way to
        // express the length on the wire.
        if state.version >= SpdmVersion::V14 {
