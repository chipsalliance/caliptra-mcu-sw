...
//!
//! Unlike the SPDM `GET_MEASUREMENTS` path — where the evidence format is fixed
//! at compile time because the requester has no field in which to express a
//! preference — this command carries an explicit `evidence_format` selector.
//! The set of formats a build can serve is still a compile-time property; the
//! wire field only selects among the formats that are already enabled.
//!
...
//!
//! Evidence can exceed one transport frame (an ML-DSA-87 PCR quote is 6388
//! bytes), so this command uses the same inline-or-large framing as
//! `EXPORT_ATTESTED_CSR`.

use caliptra_mcu_common_commands::{
    CaliptraCmdHandler, EvidenceAlgorithm, EvidenceFormat, ATTESTATION_NONCE_LEN,
...
/// `evidence_format:u32 | algorithm:u32 | nonce:u8[32]`
const GET_ATTESTATION_REQ_LEN: usize = 4 + 4 + ATTESTATION_NONCE_LEN;
...
pub(crate) fn decode_format(req: &[u8]) -> Option<(EvidenceFormat, EvidenceAlgorithm)> {
    ...
    Some((
        ...
        EvidenceAlgorithm::try_from(algorithm).ok()?,
    ))
}
    ...
    A: SpdmPalAlloc,
{
    ...
    let Ok(algorithm) = EvidenceAlgorithm::try_from(algorithm_raw) else {
        return CaliptraVdmCmdResult::Error(CaliptraCompletionCode::InvalidParameter);
    };
    let Ok(nonce) = <&[u8; ATTESTATION_NONCE_LEN]>::try_from(&req[8..]) else {
        return CaliptraVdmCmdResult::Error(CaliptraCompletionCode::InvalidPayloadSize);
    };
