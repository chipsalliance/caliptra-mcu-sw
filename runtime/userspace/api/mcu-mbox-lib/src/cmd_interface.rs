...
/// Attestation evidence can exceed every other response (an ML-DSA-87 PCR quote
/// is 6388 bytes), and it is deliberately not a [`McuMailboxResp`] variant: that
/// enum sizes *every* command's allocation by its largest variant, so folding
/// attestation in would inflate all of them.
...
async fn stage_attestation<H: CaliptraCmdHandler, Alloc: ApiAllocPool>(
    ...
    let algorithm =
        EvidenceAlgorithm::try_from(req.algorithm).map_err(|_| errors::INVALID_PARAMS)?;
    ...
    // Evidence is signed, so a truncated response is not a shorter answer, it is
    // an unverifiable one. Refuse rather than emit a `Complete` response that
    ...
    let evidence_len = handler
        .get_attestation(alloc.pool(), format, algorithm, &req.nonce, out)
        .await
        .map_err(|_| errors::MCU_MBOX_COMMON)?;
...
mod tests {
    ...
    /// Mirrors the emulator backend: the EAT is ES384-only.
    struct TestHandler;
    ...
    impl CaliptraCmdHandler for TestHandler {
        ...
        fn attestation_evidence_len(format: EvidenceFormat, algorithm: EvidenceAlgorithm) -> usize {
            match (format, algorithm) {
                (EvidenceFormat::OcpEat, EvidenceAlgorithm::EccP384) => TEST_EAT_LEN,
                (EvidenceFormat::PcrQuote, EvidenceAlgorithm::EccP384) => 1840,
                (EvidenceFormat::PcrQuote, EvidenceAlgorithm::Mldsa87) => TEST_QUOTE_MLDSA_LEN,
                _ => 0,
            }
        }
    ...
    fn evidence_is_framed_after_the_echoed_format() {
        let req = request(
            ...
            EvidenceAlgorithm::Mldsa87 as u32,
        );
    ...
    fn unsupported_pairs_are_rejected_before_generation() {
        ...
        let req = request(0xFF, EvidenceAlgorithm::EccP384 as u32);
        assert_eq!(
            block_on(stage_attestation(&TestHandler, &TestAlloc, &req, &mut body)),
