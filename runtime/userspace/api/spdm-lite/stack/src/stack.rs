...
use crate::{algorithms, capabilities, certificate, chunk, digests, vendor_defined, version};
...
mod tests {
    ...
    impl SpdmPalLargeMessage for FakePal {
        ...
        fn write_large_message(&self, _offset: usize, _data: &[u8]) -> McuResult<()> {
            Err(codes::NOT_IMPLEMENTED)
        }

        fn large_message(&self, _len: usize) -> McuResult<&[u8]> {
            Err(codes::NOT_IMPLEMENTED)
        }

        fn large_message_mut(&self, _len: usize) -> McuResult<&mut [u8]> {
            Err(codes::NOT_IMPLEMENTED)
        }
    }
