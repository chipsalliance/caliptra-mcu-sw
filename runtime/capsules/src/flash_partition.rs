...
//! This provides flash partition driver
        ...
        for cntr in self.apps.iter() {
            ...
            let started_command = cntr.enter(|app, _| {
                if app.pending_command {
                    ...
                    self.current_app.set(processid);
                    if let Ok(()) = self.userspace_call_driver(app.command, app.offset, app.length)
                    {
                        true
                    } else {
                        false
...
impl SyscallDriver for FlashPartition<'_> {
    ...
    /// - `2`: Start a read from the flash partition
    /// - `3`: Start a write to the flash partition
    /// - `4`: Start an erase of the flash partition
    fn command(
        &self,
        command_num: usize,
        offset: usize,
