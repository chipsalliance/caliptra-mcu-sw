...
impl HashEngine for HashEngineImpl {
    ...
    fn start(&mut self, hash_type: BaseHashAlgoType) -> Result<(), HashEngineError> {
        ...
        match hash_type {
            ...
            BaseHashAlgoType::TpmAlgSha384 => {
                //let _hasher = Sha384::new();
                self.ctx = Some(Hasher::Sha384(Sha384::new()));
            }
            BaseHashAlgoType::TpmAlgSha512 => {
                //let _hasher = Sha512::new();
    ...
    fn update(&mut self, data: &[u8]) -> Result<(), HashEngineError> {
        // Incremental hash
        match &mut self.ctx {
            Some(Hasher::Sha256(hasher)) => Ok(hasher.update(data)),
            Some(Hasher::Sha384(hasher)) => Ok(hasher.update(data)),
            Some(Hasher::Sha512(hasher)) => Ok(hasher.update(data)),
            _ => {
                return Err(HashEngineError::UnsupportedHashType);
            }
        }
...
mod tests {
    use super::*;
    use crate::{
        config::{self, TEST_DEVID_CERT_DER},
        protocol::cert::SpdmCertChainData,
    };
    ...
    fn test_hash_all() {
        ...
        assert_eq!(
            ...
            TEST_DEVID_CERT_DER.len() as u16 + config::TEST_ALIAS_CERT_DER.len() as u16
        );
    ...
    fn test_hash_update() {
        ...
        hash_engine.update(spdm_cert_chain_data.as_ref()).unwrap();
