...
use pldm_common::{
    ...
    protocol::firmware_update::{
        ComponentResponseCode, Descriptor, PLDM_FWUP_BASELINE_TRANSFER_SIZE, PldmFdTime
    },
};
...
pub trait FdOps {
    ...
    async fn request_update_get_transfer_size(
        &self,
...
impl<S: Syscalls> FdOps for FdOpsObject<S> {
    ...
    async fn update_component(
        ...
        if ops
            .image_loader
            .get_image_load_address(component.comp_identifier as u32)
            .await
            .is_err()
        {
            return Err(FdOpsError::UpdateComponentError);
        }
