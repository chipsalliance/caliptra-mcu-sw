...
pub mod test {
    ...
    impl<const MAX_NUM_MESSAGES:usize> TestMessages<MAX_NUM_MESSAGES> {
        fn add<Req: PldmCodec, Resp: PldmCodec>(&mut self, request: Req, response: Resp) {
            ...
            let sz = request.encode(&mut self.messages[index].request.buffer[1..]).unwrap();
            self.messages[index].request.length = sz + 1;
    
            self.messages[index].response.buffer[0] = MCTP_PLDM_COMMON_HEADER;
            let sz = response.encode(&mut self.messages[index].response.buffer[1..]).unwrap();
    ...
    pub async fn test_pldm_request_response<S: Syscalls>() {
        ...
        for i in 0..test_messages.size  {
            ...
            assert!(test_message.request.buffer[0.. length as usize] == msg_buffer[0..length as usize]);
            
            mctp_pldm.send_response(&test_message.response.buffer[..test_message.response.length], info).await.unwrap();
