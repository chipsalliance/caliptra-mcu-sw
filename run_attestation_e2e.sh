...
pushd "$WORKSPACE" >/dev/null
cargo t -p tests-integration -- --test test_mctp_spdm_attestation --nocapture --include-ignored 2>&1 | tee "$SPDM_TEST_LOG" | grep -Ev 'no prebuilt available|process binary in flash|caliptra-okref|MCTP-SPDM-RESPONDER-VALIDATOR' | grep -E '(running|test |ok|FAILED|PASS|verify_measurement|SPDM_TASK|SPDM main)' || true
popd >/dev/null
...
export SIGNED_CORIM_PATH="$SIGNED_CORIM_DIR"
