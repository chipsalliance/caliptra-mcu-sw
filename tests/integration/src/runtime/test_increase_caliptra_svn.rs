...
fn test_increase_caliptra_svn_max() -> Result<()> {
    ...
    // Check SVN value at the RUNTIME_SVN fuse offset using specific decoding logic.
    // The SVN is represented as a bitmask where the number of set bits is the SVN.
    let svn_offset =
        caliptra_mcu_registers_generated::fuses::OTP_CPTRA_CORE_RUNTIME_SVN.byte_offset;
    let svn_bytes = &otp[svn_offset..svn_offset + 16];
    ...
    // Retry a few times if the Caliptra mailbox is still locked (busy from boot).
    let fw_info_id = caliptra_api::mailbox::CommandId::FW_INFO.into();
    let payload = caliptra_api::mailbox::MailboxReqHeader {
        chksum: calc_checksum(fw_info_id, &[]),
    };
