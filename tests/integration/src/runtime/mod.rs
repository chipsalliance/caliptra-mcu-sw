...
/// Sign with an arbitrary keypair; the tail carries that keypair's own public
/// keys (so an unauthorized keypair is caught only by the device anchor gate).
pub fn sign_auth_cmd_challenge_with(
    ...
    let authorizer = AsymmetricCommandAuthorizer::new(ecc_priv, mldsa_seed)?;
...
/// Send caller-assembled bytes (header re-checksummed); used by the replay test
/// to resend a valid frame without fetching a fresh challenge.
pub fn send_authorized_bytes<R: caliptra_mcu_mbox_common::messages::Request>(
...
/// Like `authorize_cmd`, but runs `tamper` on the signed wire bytes before
/// checksum + send, so a negative test can corrupt one field.
pub fn authorize_cmd_with(
    hw: &mut impl McuHwModel,
    cmd_id: u32,
    cmd: &[u8],
...
/// Tampering variant of `execute_authorized_req`: signs correctly, lets `tamper`
/// mutate the bytes, re-checksums (so the frame is transport-valid), and sends.
pub fn execute_authorized_req_tampered<R: caliptra_mcu_mbox_common::messages::Request>(
    ...
    // Authorize (sign), then let the caller corrupt exactly one field.
    let mut auth_cmd = authorize_cmd_with(hw, u32::from(R::ID), req_bytes, tamper)?;

    // Populate the request checksum over body + MAC (post-tamper, so transport is valid).
    let checksum = calc_checksum(R::ID.into(), &auth_cmd[size_of::<i32>()..]);
    let hdr: &mut MailboxReqHeader =
        MailboxReqHeader::mut_from_bytes(&mut auth_cmd[..size_of::<MailboxReqHeader>()]).unwrap();
    hdr.chksum = checksum;
