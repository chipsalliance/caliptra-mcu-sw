...
mod test {
    ...
    fn test_nwp_exception() {
        ...
        let mut hw = start_runtime_hw_model(TestParams {
            ...
            rom_only: true,
            network_rom_feature: Some("test-exception"),
            ..Default::default()
        });
        ...
        assert!(
            output.contains("NWP exception test: triggering"),
            "Should print pre-exception message"
        );
        assert!(
            output.contains("Ec="),
            "Exception handler should fire and print Ec=<cause>@<pc>"
        );
