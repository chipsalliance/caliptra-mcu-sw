...
pub mod test {
    ...
    impl RequestResponseTest {
        ...
        fn process_message_with_options(
            ...
            loop {
                match self.mbox.get_execute_response() {
                    Ok(resp) => {
                        println!(
                            "[process_message] cmd={:#X} completed after {} polls, status_code={}, data_len={}",
                            cmd, poll_count, resp.status_code, resp.data.len()
                        );
                        return Ok(resp);
                    }
                    Err(McuMailboxError::Busy) => {
                        ...
                        if emulator_ticks_elapsed(start, timeout_ticks) {
                            println!(
                                "[process_message] Timeout waiting for response for MCU mailbox cmd: {:#X} after {} polls",
                                cmd, poll_count
                            );
                    ...
                    Err(e) => {
                        if continue_on_error {
                            ...
                            if emulator_ticks_elapsed(start, timeout_ticks) {
                                println!(
                                    "[process_message] Timeout (continue_on_error) for MCU mailbox cmd: {:#X}, last_error={:?}, polls={}",
                                    cmd, e, poll_count
                                );
                                return Err(McuMailboxError::Timeout);
                            }
                            ...
                            println!(
                                "[process_message] Error for MCU mailbox cmd: {:#X}, error={:?}, polls={}",
                                cmd, e, poll_count
                            );
                            return Err(e);
                        }
        ...
        fn test_send_receive(&mut self, feature: &str) -> Result<(), ()> {
            ...
            for (idx, message_pair) in test_messages.iter().enumerate() {
                println!(
                    "[test_send_receive] Sending message {}/{}: cmd={:#X}, request_len={}",
                    idx + 1,
                    test_messages.len(),
                    message_pair.cmd,
                    message_pair.request.len()
                );
                let actual_response = self
                    .process_message(message_pair.cmd, &message_pair.request)
                    .map_err(|e| {
                        println!(
                            "[test_send_receive] FAILED on message {}/{}: cmd={:#X}, error={:?}",
                            idx + 1,
                            test_messages.len(),
                            message_pair.cmd,
                            e
                ...
                if actual_response.data != message_pair.response {
                    println!(
                        "[test_send_receive] MISMATCH on message {}/{}: cmd={:#X}, expected_len={}, actual_len={}, status_code={}",
                        idx + 1,
                        test_messages.len(),
                        message_pair.cmd,
                        message_pair.response.len(),
                        actual_response.data.len(),
                        actual_response.status_code
                    );
                    println!(
                        "[test_send_receive] Expected (first 32 bytes): {:?}",
                        &message_pair.response[..message_pair.response.len().min(32)]
                    );
                    println!(
                        "[test_send_receive] Actual   (first 32 bytes): {:?}",
                        &actual_response.data[..actual_response.data.len().min(32)]
                    );
                }
        ...
        /// These commands are forwarded from the MCU mailbox to the Caliptra mailbox.
        ///
        /// The production debug unlock commands (PRODUCTION_AUTH_DEBUG_UNLOCK_REQ and
        /// PRODUCTION_AUTH_DEBUG_UNLOCK_TOKEN) are supported by the Caliptra runtime,
        /// but require the device to be in Production lifecycle with proper key
        /// provisioning. In this test configuration (default lifecycle), the commands
        /// will be dispatched but rejected by Caliptra.
        ///
        /// The manufacturing debug unlock token (MANUF_DEBUG_UNLOCK_REQ_TOKEN) is
        /// ROM-only and not supported by the Caliptra runtime.
        ///
        /// These tests verify the MCU mailbox passthrough correctly dispatches the
        /// commands to the Caliptra mailbox and returns a response.
        fn add_debug_unlock_tests(&mut self) -> Result<(), ()> {
            println!("Running debug unlock passthrough tests");

            // Test 1: Manufacturing debug unlock token (ROM-only command)
            ...
            match manuf_resp {
                Ok(resp) => {
                    println!(
                        "    Manuf debug unlock token: got response (status={})",
                        resp.status_code
                    );
                    assert_eq!(
                        resp.status_code,
                        MbxCmdStatus::Failure as u32,
                        "Manuf debug unlock should fail in runtime (ROM-only command)"
                    );
                }
                Err(e) => {
                    // Transport-level rejection is also acceptable for unsupported commands
                    println!("    Manuf debug unlock token: got error {:?} (expected for ROM-only command)", e);
                }
            }
            println!("  Manuf debug unlock token passthrough test passed");

            // Test 2: Production debug unlock request
            ...
            // Test 3: Production debug unlock token
            // This command is supported by the Caliptra runtime but requires
        ...
        fn add_prod_debug_unlock_e2e_test(
            ...
            let token = mcu_hw_model::debug_unlock::prod_debug_unlock_gen_signed_token(
                &challenge,
                unlock_level,
                ecc_private_key,
                mldsa_private_key,
