...
pub struct FlashPage(pub [u8; PAGE_SIZE]);
...
impl Index<usize> for FlashPage {
    type Output = u8;
...
impl IndexMut<usize> for FlashPage {
...
pub struct MailboxFlashCtrl<'a> {
    ...
    read_buf: TakeCell<'static, FlashPage>,
    write_buf: TakeCell<'static, FlashPage>,
    pending_op: OptionalCell<FlashOperation>,
    //pending_page: OptionalCell<usize>,
...
impl<'a> MailboxFlashCtrl<'a> {
    pub fn new(registers: StaticRef<mci::regs::Mci>) -> MailboxFlashCtrl<'a> {
        MailboxFlashCtrl {
            ...
            //  pending_page: OptionalCell::empty(),
