...
const MAX_STREAMED_SPDM_REQUEST_LEN: usize = {
    let mut required = 0;
    if MAX_STREAMED_VDM_REQUEST_LEN > required {
        required = MAX_STREAMED_VDM_REQUEST_LEN;
    }
    if MAX_STREAMED_SET_CERTIFICATE_REQUEST_LEN > required {
        required = MAX_STREAMED_SET_CERTIFICATE_REQUEST_LEN;
    }
    required
};
...
/// Maximum streamed SET_CERTIFICATE request for one managed endorsement slot.
///
/// The CertChain capacity is derived from the certificate-store partition and
/// configured managed endorsement count, then capped by the standard u16 field.
...
const MAX_STREAMED_SET_CERTIFICATE_REQUEST_LEN: usize = caliptra_mcu_spdm_codec::SpdmMsgHdrPdu::SIZE
    ...
    + crate::cert_store::MAX_MANAGED_ENDORSEMENT_CERT_CHAIN_LEN;
#[cfg(not(feature = "test-mctp-spdm-set-certificate"))]
const MAX_STREAMED_SET_CERTIFICATE_REQUEST_LEN: usize = 0;
