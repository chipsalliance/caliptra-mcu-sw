...
use caliptra_mcu_common_commands::{
    ...
    DeviceCapabilities, DeviceId, DeviceInfo, FirmwareVersion,
};
...
impl CaliptraCmdHandler for CaliptraCmdBackend {
    ...
    async fn request_debug_unlock(
        ...
        .await
        .map_err(|e| {
            let mut w = caliptra_mcu_libtock_console::Console::<caliptra_mcu_libsyscall_caliptra::DefaultSyscalls>::writer();
            let _ = core::fmt::Write::write_fmt(&mut w, format_args!("[MCU] debug_unlock_req mailbox failed: err={:?}\n", e));
...

        {
            ...
            let _ = core::fmt::Write::write_fmt(&mut w, format_args!(
                "[MCU] debug_unlock_req OK: UDI[0..4]={:02x?}, challenge[0..4]={:02x?}\n",
                &resp.unique_device_identifier[..4],
                &resp.challenge[..4]
            ));
        }
    ...
    async fn authorize_debug_unlock_token(
...

        {
            let mut w = caliptra_mcu_libtock_console::Console::<caliptra_mcu_libsyscall_caliptra::DefaultSyscalls>::writer();
            let _ = core::fmt::Write::write_fmt(&mut w, format_args!(
                "[MCU] debug_unlock_token: sending {} bytes token\n", token_data.len()
            ));
        }
        ...
        .map_err(|e| {
            let mut w = caliptra_mcu_libtock_console::Console::<caliptra_mcu_libsyscall_caliptra::DefaultSyscalls>::writer();
            let _ = core::fmt::Write::write_fmt(&mut w, format_args!("[MCU] debug_unlock_token mailbox failed: err={:?}\n", e));
            CaliptraCompletionCode::OperationFailed
        })?;
...
static STREAM_REMAINDER: embassy_sync::mutex::Mutex<
    ...
    StreamRemainder,
> = embassy_sync::mutex::Mutex::new(StreamRemainder { buf: [0; 3], len: 0 });
...
async fn send_aligned(
    mailbox: &caliptra_mcu_libsyscall_caliptra::mailbox::Mailbox<caliptra_mcu_libsyscall_caliptra::DefaultSyscalls>,
    data: &[u8],
    ...
    while offset < total {
        ...
        buf[buf_start..buf_start + can_fill].copy_from_slice(&data[src_offset..src_offset + can_fill]);
        src_offset += can_fill;
...
impl caliptra_mcu_spdm_lib::vdm_handler::VdmStreamHandler for CaliptraCmdBackend {
    ...
    async fn stream_init(
        ...
        let mailbox = caliptra_mcu_libsyscall_caliptra::mailbox::Mailbox::<caliptra_mcu_libsyscall_caliptra::DefaultSyscalls>::new();
