...
use coset::iana::*;
use coset::RegisteredLabel::Assigned;
...
use ocp_eat::profile_config::{OCP_EAT_CLAIMS_KEY_ID, OCP_ENVELOPE_CSR_KEY_ID};
...
pub async fn test_ocp_eat_cwt() {
    ...
    let mut ocp_cwt_slice = [0u8; 2048];

    let ocp_cwt_size = match ocp_cwt.generate_ocp_eat(&mut ocp_cwt_slice).await {
        Ok(size) => size,
        Err(err) => {
