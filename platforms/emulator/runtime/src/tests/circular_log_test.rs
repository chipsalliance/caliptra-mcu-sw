    ...
    fn run(&self) {
        match self.state.get() {
            TestState::Operate => {
                ...
                romtime::println!("[xs debug]Starting test index {:?} ops {:?}", op_index, self.ops[op_index]);
...
impl<A: Alarm<'static>> LogWriteClient for LogTest<A> {
    fn append_done(
        ...
        romtime::println!("[xs debug][test append_done] buf_length={} records_lost={} error={:?}", length, records_lost, error);
        match error {
            Ok(()) => {
                if length != BUFFER_LEN {
                    panic!(
        ...
        self.wait();
        romtime::println!("[xs debug][test append_done] done, write_val={}", self.write_val.get());
    }
