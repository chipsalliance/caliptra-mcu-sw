...
pub fn generate_tbf_header(
    mut caliptra_mcu_tbf_header: TbfHeader,
    instruction_block: Memory,
    elf: PathBuf,
    binary: PathBuf,
) -> Result<Vec<u8>> {
    // Determine the header and binary size of the application.
    let header_size: u32 = caliptra_mcu_tbf_header
        .generate()?
        .get_ref()
        .len()
        .try_into()?;
    ...
    caliptra_mcu_tbf_header.set_total_size(total_size);
    caliptra_mcu_tbf_header.set_binary_end_offset(total_size);
    caliptra_mcu_tbf_header.set_init_fn_offset(init_fn_offset);
    caliptra_mcu_tbf_header
        .generate()
        .map(|h| h.into_inner())
        .map_err(|e| e.into())
}
