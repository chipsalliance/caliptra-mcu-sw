...
fn dynamically_size(
    ...
    // A utility to increase the address to match the alignment requirement.
    let align = |addr| {
        if addr % ALIGNMENT != 0 {
    ...
    manifest.kernel.exec_mem = Some(AllocationRequest {
        size: align(sizes.kernel.instructions),
        alignment: None,
    });
    manifest.kernel.data_mem = Some(AllocationRequest {
        size: align(sizes.kernel.data),
        alignment: None,
    });
    ...
    manifest
        ...
        .try_for_each(|(manifest_app, size_app)| {
            ...
            manifest_app.exec_mem = Some(AllocationRequest {
                size: align(size_app.instructions),
                alignment: None,
            });

            manifest_app.data_mem = Some(AllocationRequest {
                size: align(size_app.data),
                alignment: None,
            });
