...
def parse_evidence_measurements(data, offset):
    ...
    try:
        ...
        for j in range(measurements_count):
            meas_info, current_offset = parse_cbor_header_with_names(
                data, current_offset
            )
                ...
                for k in range(meas_array_count):
                    ...
                    if elem_info[0] == "map":
                        ...
                        for m in range(elem_info[1]):
                            ...
                            val_info, current_offset = parse_cbor_header_with_names(
                                data, current_offset
                            )
                            result += f"\n              {key_info} -> {val_info}"
    ...
    try:
        ...
        for m in range(map_count):
            ...
            if key_info[0] == "text_string":
                key_value = data[current_offset : current_offset + key_info[1]].decode(
                    "utf-8", errors="ignore"
                )
...
def parse_class_map_fields(data, offset, inner_map_count):
    ...
    try:
        ...
        for n in range(inner_map_count):
            inner_key_info, current_offset = parse_cbor_header_with_names(
                data, current_offset
            )
            inner_key_value = None
            if inner_key_info[0] == "text_string":
                inner_key_value = data[
                    current_offset : current_offset + inner_key_info[1]
            ...
            inner_val_info, current_offset = parse_cbor_header_with_names(
                data, current_offset
            )
            ...
            if inner_val_info[0] == "text_string":
                ...
                tagged_content, current_offset = parse_cbor_header_with_names(
                    data, current_offset
                )
...
def parse_measurement_array(data, offset, array_elem_count):
    ...
    try:
        ...
        for j in range(array_elem_count):
            measurement_info, current_offset = parse_cbor_header_with_names(
                data, current_offset
            )

            if measurement_info[0] == "map":
                # Parse measurement map with detailed structure
                meas_result, current_offset = parse_measurement_map_detailed(
                    data, current_offset, measurement_info[1], j
                )
                result += f"\n              {meas_result}"
            elif measurement_info[0] == "array":
                # Parse nested measurement arrays
                nested_result, current_offset = parse_nested_measurement_array(
                    data, current_offset, measurement_info[1], j
                )
                result += f"\n              {nested_result}"
            elif measurement_info[0] == "text_string":
                text_val = data[
                    current_offset : current_offset + measurement_info[1]
                ].decode("utf-8", errors="replace")
...
def parse_measurement_map_detailed(data, offset, meas_map_count, meas_index):
    ...
    try:
        ...
        for mm in range(meas_map_count):
            ...
            meas_key_info, current_offset = parse_cbor_header_with_names(
                data, current_offset
            )
            if meas_key_info[0] == "positive_int":
                ...
                meas_key_name = data[
                    current_offset : current_offset + meas_key_info[1]
                ].decode("utf-8", errors="replace")
            ...
            meas_val_info, current_offset = parse_cbor_header_with_names(
                data, current_offset
            )

            if meas_val_info[0] == "map":
                # Parse the map contents (firmware measurement details)
                mval_result, current_offset = parse_measurement_value_map(
                    data, current_offset, meas_val_info[1], meas_key_name
                )
                result += f"\n                {mval_result}"
                ...
                meas_val_str = data[
                    current_offset : current_offset + meas_val_info[1]
                ].decode("utf-8", errors="replace")
                current_offset += meas_val_info[1]
                result += f"\n                {meas_key_name}: '{meas_val_str}'"
...
def parse_measurement_value_map(data, offset, mval_map_count, field_name):
    ...
    try:
        ...
        for mv in range(mval_map_count):
            # Parse map key
            mv_key_info, current_offset = parse_cbor_header_with_names(
                data, current_offset
            )
            if mv_key_info[0] == "positive_int":
                ...
                mv_key_name = data[
                    current_offset : current_offset + mv_key_info[1]
                ].decode("utf-8", errors="replace")
            ...
            # Parse map value
            mv_val_info, current_offset = parse_cbor_header_with_names(
                data, current_offset
            )
...
def parse_measurement_value_array(data, offset, mv_array_count, field_name):
    ...
    try:
        ...
        for mvx in range(mv_array_count):
            mvx_info, current_offset = parse_cbor_header_with_names(
                data, current_offset
            )

            if mvx_info[0] == "text_string":
                mvx_val = data[current_offset : current_offset + mvx_info[1]].decode(
...
def parse_nested_array_elements(data, offset, mvx_sub_count, parent_index):
    ...
    try:
        ...
        for mvxs in range(mvx_sub_count):
            mvxs_info, current_offset = parse_cbor_header_with_names(
                data, current_offset
            )
...
def parse_integrity_registers_map(data, offset, ir_map_count):
    ...
    try:
        ...
        for ir in range(ir_map_count):
            ...
            reg_id_info, current_offset = parse_cbor_header_with_names(
                data, current_offset
            )
            reg_id_name = "unknown"
            if reg_id_info[0] == "positive_int":
                ...
                reg_id_name = data[
                    current_offset : current_offset + reg_id_info[1]
                ].decode("utf-8", errors="replace")
                current_offset += reg_id_info[1]

            # Parse digests array (value)
            reg_val_info, current_offset = parse_cbor_header_with_names(
                data, current_offset
            )
...
def parse_nested_measurement_array(data, offset, meas_sub_array_count, meas_index):
    ...
    try:
        ...
        for msub in range(meas_sub_array_count):
            msub_info, current_offset = parse_cbor_header_with_names(
                data, current_offset
            )

            if msub_info[0] == "text_string":
                msub_val = data[current_offset : current_offset + msub_info[1]].decode(
                    "utf-8", errors="replace"
                )
...
def parse_simple_map(data, offset, msub_map_count, elem_index):
    ...
    try:
        ...
        for msmap in range(msub_map_count):
            msmap_key_info, current_offset = parse_cbor_header_with_names(
                data, current_offset
            )
            msmap_val_info, current_offset = parse_cbor_header_with_names(
                data, current_offset
            )
            ...
            if msmap_key_info[0] == "positive_int":
                ...
                msmap_key_name = data[
                    current_offset - msmap_key_info[1] : current_offset
                ].decode("utf-8", errors="replace")
        ...
        for _ in range(map_pairs):  # key-value pairs
            ...
            if value_header and value_header[0] == 2:  # Byte string
                ...
                if analyze_certificate_headers(key_value, value_data):
                    ...
                    print(
                        f"    Data: {value_data[:16].hex()}{'...' if len(value_data) > 16 else ''}"
                    )
...
def parse_eat_claims(payload):
    ...
    for i in range(num_claims):
        ...
        if key == 273 and value_type == 4:  # Measurements array
            ...
            claims_offset = parse_generic_claim_value(
                payload, new_offset, value_type, value_info
            )
...
def parse_cose_payload_with_data(data, offset):
    ...
    header, new_offset = parse_cbor_header(data, offset)
    if not (header and header[0] == 2):  # Byte string
        print(f"Skipping payload because header: {header}")
        print(data[new_offset : new_offset + header[1]])
        return offset, None
...
def parse_cose_sign1_structure(cose_data):
    ...
    offset, protected_headers = parse_cose_protected_headers_with_data(
        cose_data, offset
    )
    ...
    if all(
        key in validation_data and validation_data[key] is not None
