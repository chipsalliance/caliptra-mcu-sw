...
use caliptra_mcu_builder::{
    runtime_build_with_apps, CaliptraBuildArgs, CaliptraBuilder, PROJECT_ROOT,
};
...
pub(crate) fn runtime_run(args: Commands) -> Result<()> {
    ...
    let dccm_offset = format!(
        "0x{:x}",
        dccm_offset.unwrap_or(caliptra_mcu_config_emulator::EMULATOR_MEMORY_MAP.dccm_offset),
    );
    cargo_run_args.extend(["--dccm-offset", &dccm_offset]);
    let dccm_size = format!(
        "0x{:x}",
        dccm_size.unwrap_or(caliptra_mcu_config_emulator::EMULATOR_MEMORY_MAP.dccm_size)
    );
