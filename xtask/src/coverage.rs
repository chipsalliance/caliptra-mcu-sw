...
pub(crate) fn coverage(analyze_only: bool) -> Result<()> {
    ...
    // The ELF is the same path without ".bin" extension
    let rom_elf = rom_path.with_extension("");
    ...
    let paths = mcu_coverage::get_bitvec_paths(cov_dir.to_str().unwrap())
    ...
    let cv = mcu_coverage::CoverageMap::new(paths);

    // ROM coverage
    if rom_elf.exists() {
        let elf_bytes = std::fs::read(&rom_elf)?;
        let rom_image = std::fs::read(&rom_path)?;
        let rom_tag = mcu_coverage::get_tag_from_image(&rom_image);

        if let Some(bv) = cv.map.get(&rom_tag) {
            let (load_addr, instr_pcs) = mcu_coverage::collect_instr_pcs_from_elf(&elf_bytes)?;
            let (hit, total) =
                mcu_coverage::coverage_from_bitmap(load_addr as usize, bv, &instr_pcs);
            ...
            let uncovered = mcu_coverage::uncovered_functions(load_addr as usize, &elf_bytes, bv)?;
            for f in &uncovered {
                println!("{}", f);
            }
        } else {
        ...
        println!("MCU ROM ELF not found at {}", rom_elf.display());
    }

    // ICCM/SRAM coverage - report any remaining tags
    for (tag, bv) in &cv.map {
        let rom_image = std::fs::read(&rom_path).ok();
        let rom_tag = rom_image
            .as_ref()
            .map(|img| mcu_coverage::get_tag_from_image(img));
