...
/// These peripherals use a separate implementation (e.g., TAP-based networking).
static ROOTBUS_SKIP_TYPES: LazyLock<HashSet<&str>> = LazyLock::new(|| {
    HashSet::from([
        "ethernet", // Network coprocessor ethernet - uses separate TAP-based implementation
    ])
});
...
fn emu_make_root_bus<'a>(
    ...
    for block in blocks_sorted {
        let rblock = block.block();
        if SKIP_TYPES.contains(rblock.name.as_str()) || ROOTBUS_SKIP_TYPES.contains(rblock.name.as_str()) {
            continue;
        }
