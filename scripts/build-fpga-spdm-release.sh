...
build_firmware() {
    ...
    local RUNTIME_FEATURES="test-mctp-spdm-attestation,test-mctp-spdm-responder-conformance,test-exit-immediately"
...
if release_exists "${SPDM_EMU_TAG}"; then
    ...
    build_spdm_emu
    echo "skipping spdm-emu build for now, using pre-built artifacts from /tmp/spdm-emu-binaries.tar.gz (delete it to force rebuild)"
fi
...
if release_exists "${RELEASE_TAG}"; then
    log "Firmware release '${RELEASE_TAG}' already exists — deleting and re-creating..."
    gh release delete "${RELEASE_TAG}" --repo "${REPO}" --yes
    git push --delete "https://github.com/${REPO}.git" "${RELEASE_TAG}" 2>/dev/null || true
fi
