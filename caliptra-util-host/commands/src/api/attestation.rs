...
use caliptra_mcu_core_util_host_command_types::{
    attestation::{
        EvidenceAlgorithm, EvidenceFormat, GetAttestationRequest, GetAttestationResponse,
    },
    CaliptraCommandId,
};
...
///   support the requested format/algorithm pair
pub fn caliptra_cmd_get_attestation(
    ...
    let request = GetAttestationRequest::new(format, algorithm, nonce);
    session
