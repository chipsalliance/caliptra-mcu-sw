...
use caliptra_command_types::{CommandRequest, CommandResponse, CommandError, CommandResult};
use zerocopy::{AsBytes, FromBytes};

/// Pack a command request into a fixed buffer using zerocopy
pub fn pack_command_request<Req: CommandRequest>(request: &Req, buffer: &mut [u8]) -> CommandResult<usize> {
    let data = AsBytes::as_bytes(request);
...
pub fn unpack_command_request<Req: CommandRequest>(data: &[u8]) -> CommandResult<Req> {
    FromBytes::read_from(data).ok_or(CommandError::DeserializationError)
}

/// Pack a command response into a fixed buffer using zerocopy
pub fn pack_command_response<Resp: CommandResponse>(response: &Resp, buffer: &mut [u8]) -> CommandResult<usize> {
    let data = AsBytes::as_bytes(response);
    if buffer.len() < data.len() {
        return Err(CommandError::BufferTooSmall);
    }
    buffer[..data.len()].copy_from_slice(data);
