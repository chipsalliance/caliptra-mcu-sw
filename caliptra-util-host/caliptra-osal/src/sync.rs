...
impl<T> Mutex<T> {
    ...
    pub fn lock(&self) -> OsalResult<std_sync::MutexGuard<'_, T>> {
        self.inner.lock().map_err(|_| OsalError::ResourceUnavailable)
    }
...
impl<T> RwLock<T> {
    ...
    pub fn read(&self) -> OsalResult<std_sync::RwLockReadGuard<'_, T>> {
        self.inner.read().map_err(|_| OsalError::ResourceUnavailable)
    }
    
    #[cfg(feature = "std")]
    pub fn write(&self) -> OsalResult<std_sync::RwLockWriteGuard<'_, T>> {
        self.inner.write().map_err(|_| OsalError::ResourceUnavailable)
    }
}
...
impl Condvar {
    ...
    #[cfg(feature = "std")]
    pub fn wait<'a, T>(&self, guard: std_sync::MutexGuard<'a, T>) -> OsalResult<std_sync::MutexGuard<'a, T>> {
        self.inner.wait(guard).map_err(|_| OsalError::ResourceUnavailable)
    }
