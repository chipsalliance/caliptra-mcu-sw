...
impl ThreadHandle {
    ...
    pub fn join(mut self) -> OsalResult<()> {
        if let Some(handle) = self.inner.take() {
            handle.join().map_err(|_| OsalError::Other("Thread join failed"))?;
        }
        Ok(())
    }
...
pub fn init(max_threads: usize, default_stack_size: usize) -> OsalResult<()> {
    #[cfg(feature = "std")]
    {
        static STD_THREAD: StdThread = StdThread;
        unsafe {
            THREAD_IMPL = Some(&STD_THREAD);
        }
    }
...
fn get_thread() -> &'static dyn Thread {
    unsafe {
        THREAD_IMPL.expect("Thread subsystem not initialized")
    }
}
