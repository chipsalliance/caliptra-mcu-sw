...
impl Instant {
    ...
    pub fn checked_add(&self, duration: Duration) -> Option<Self> {
        self.nanos.checked_add(duration.as_nanos()).map(|nanos| Self { nanos })
    }
    
    /// Subtract duration from instant
    pub fn checked_sub(&self, duration: Duration) -> Option<Self> {
        self.nanos.checked_sub(duration.as_nanos()).map(|nanos| Self { nanos })
    }
}
...
pub trait Timer: Send + Sync {
    ...
    fn schedule_once(&self, duration: Duration, callback: Box<dyn FnOnce() + Send>) -> OsalResult<TimerHandle>;
    
    /// Schedule a repeating timer
    fn schedule_repeat(&self, duration: Duration, callback: Box<dyn Fn() + Send + Sync>) -> OsalResult<TimerHandle>;
...
impl StdTimer {
    ...
    fn next_handle(&self) -> TimerHandle {
        let handle = self.next_handle.fetch_add(1, core::sync::atomic::Ordering::SeqCst);
        TimerHandle(handle)
    }
}
...
pub fn init(resolution_us: u64) -> OsalResult<()> {
    #[cfg(feature = "std")]
    {
...
fn get_timer() -> &'static dyn Timer {
    unsafe {
        TIMER.expect("Time subsystem not initialized")
    }
}
