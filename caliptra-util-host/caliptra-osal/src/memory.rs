...
pub trait Allocator: Send + Sync {
    ...
    /// Reallocate memory
    unsafe fn realloc(&self, ptr: *mut u8, old_size: usize, new_size: usize) -> OsalResult<*mut u8> {
...
impl Allocator for StdAllocator {
    fn alloc(&self, size: usize) -> OsalResult<*mut u8> {
        ...
        let layout = Layout::from_size_align(size, 8)
            .map_err(|_| OsalError::InvalidParameter)?;
...
pub fn init(pool_size: usize) -> OsalResult<()> {
    #[cfg(feature = "std")]
    {
        static STD_ALLOCATOR: StdAllocator = StdAllocator;
        unsafe {
            ALLOCATOR = Some(&STD_ALLOCATOR);
        }
    }
...
fn get_allocator() -> &'static dyn Allocator {
    unsafe {
        ALLOCATOR.expect("Memory subsystem not initialized")
    }
}
