...
use anyhow::Result;
use caliptra_mcu_command_authorizer::CommandAuthorizer;
...
pub struct Validator {
    ...
    command_authorizer: Option<Box<dyn CommandAuthorizer>>,
}
...
impl Validator {
    ...
    pub fn set_command_authorizer(mut self, authorizer: Box<dyn CommandAuthorizer>) -> Self {
...
impl Validator {
    ...
    ///    using the configured `CommandAuthorizer`
    /// 3. Submits the FE_PROG command with the MAC
    ///
    /// If no `CommandAuthorizer` is configured, the test is skipped.
    fn validate_fe_prog(&self, client: &mut MailboxClient) -> ValidationResult {
        ...
        let authorizer = match self.command_authorizer.as_ref() {
            Some(a) => a,
            None => {
                println!("⊘ FeProg validation SKIPPED (no CommandAuthorizer configured)");
                return ValidationResult {
                    test_name,
                    passed: true,
                    error_message: None,
        ...
        // Step 2: Compute MAC via the CommandAuthorizer trait
        ...
        let mac_bytes = match mac_result {
            Ok(m) => m,
            Err(e) => {
                let error_str = format!("CommandAuthorizer::authorize failed: {}", e);
