...
impl AsymAlgo {
    ...
    pub const fn name(self) -> &'static str {
        match self {
            AsymAlgo::EccP384 => "EccP384",
            AsymAlgo::Mldsa87 => "Mldsa87",
        }
    }
}
...
///
/// Mirrors the firmware `PkiEntitySlot`. Selects an endorsement hierarchy, not
/// a signing key: every slot signs with the same DPE leaf key today.
///
/// Host tooling only ever sends `Vendor`; `Owner` documents the wire encoding.
#[repr(u32)]
...
pub struct GetAttestationRequest {
    ...
    pub algorithm: u32,
    /// PKI entity whose hierarchy signs (0=Vendor, 1=Owner)
...
impl GetAttestationRequest {
    ...
    ///
    /// Always names [`PkiEntitySlot::Vendor`], the only entity devices honor.
    pub fn new(
    ...
    ///
    /// Devices return the bitmap without decoding the algorithm, entity, or
    /// nonce fields. They are set to defined values anyway so the request is
    /// well-formed on the wire.
    pub fn query() -> Self {
        Self {
            evidence_format: EVIDENCE_FORMAT_QUERY,
            algorithm: AsymAlgo::EccP384 as u32,
...
mod tests {
    ...
    fn request_layout_matches_device_contract() {
        // 4 (format) + 4 (algorithm) + 4 (pki_entity_slot) + 32 (nonce)
        assert_eq!(core::mem::size_of::<GetAttestationRequest>(), 44);

        // Field order is the wire order the device decodes. The nonce landing
        // at offset 12 is what pins pki_entity_slot into the gap before it.
        let req = GetAttestationRequest::new(
            EvidenceFormat::PcrQuote,
            AsymAlgo::Mldsa87,
            PkiEntitySlot::Vendor,
            &[0x5A; 32],
        );
        ...
        assert_eq!(
            ...
            AsymAlgo::Mldsa87 as u32
        );
        assert_eq!(
            u32::from_le_bytes(bytes[8..12].try_into().unwrap()),
            PkiEntitySlot::Vendor as u32
        );
        assert_eq!(&bytes[12..44], &[0x5A; 32]);
    }

    #[test]
    fn vendor_is_the_only_entity_host_tooling_sends() {
        let req = GetAttestationRequest::new(
            EvidenceFormat::OcpEat,
            AsymAlgo::EccP384,
            PkiEntitySlot::Vendor,
            &[0; 32],
        );
        assert_eq!(req.pki_entity_slot, PkiEntitySlot::Vendor as u32);
        assert_eq!(
            GetAttestationRequest::query().pki_entity_slot,
            PkiEntitySlot::Vendor as u32
        );
    }
