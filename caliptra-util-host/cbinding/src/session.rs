...
use caliptra_util_host_transport::{Transport, Mailbox};
...
fn session_error_to_caliptra_error(err: caliptra_util_host_session::SessionError) -> CaliptraError {
    match err {
        ...
        caliptra_util_host_session::SessionError::ConfigurationError(_) => CaliptraError::InvalidArgument,
...
unsafe fn transport_from_c_pointer_with_protocol(
    ...
    match protocol_type {
        ...
        CaliptraProtocolType::Custom => {
            ...
            let custom_transport = *Box::from_raw(transport_ptr as *mut crate::custom_transport::CTransportWrapper);
...
pub extern "C" fn caliptra_session_create_with_protocol(
    ...
    let boxed_transport = match unsafe { transport_from_c_pointer_with_protocol(transport, protocol_type) } {
        Ok(bt) => bt,
        Err(err) => return err,
    };

    
    let transport_ref: &'static mut dyn Transport = unsafe {core::mem::transmute(boxed_transport)};
