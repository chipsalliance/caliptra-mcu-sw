...
impl MockMailbox {
    ...
    fn process_command(&mut self, external_cmd: u32, _payload: &[u8]) -> Result<&[u8], MailboxError> {
        ...
        match external_cmd {
            ...
            0x4D44_494E => { // MC_DEVICE_INFO ("MDIN")
                ...
                self.response_buffer[0..8].copy_from_slice(&[0x01, 0x00, 0x00, 0x00, 0x00, 0x10, 0x00, 0x00]);
