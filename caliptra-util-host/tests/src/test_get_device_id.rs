...
use crate::common::{MockMailbox, test_constants::*};
...
fn test_get_device_id_basic() {
    ...
    println!("Testing with expected device ID: 0x{:04X}", expected_device_id);
    
    // Create mock mailbox and mailbox transport
    let mut mock_mailbox = MockMailbox::new(expected_device_id);
    let mut mailbox_transport = Mailbox::new(&mut mock_mailbox as &mut dyn caliptra_util_host_transport::transports::mailbox::MailboxDriver);
    
    // Create and connect session
    let mut session = CaliptraSession::new(1, &mut mailbox_transport as &mut dyn caliptra_util_host_transport::Transport)
    ...
    match device_id_result {
        Ok(device_id) => {
            println!("Successfully got device ID via high-level API: {:?}", device_id);
            ...
            println!("Device ID verified: 0x{:04X} matches expected: 0x{:04X}", 
                device_id.device_id, expected_device_id);
                
            // Also verify other fields are set correctly
            assert_eq!(device_id.vendor_id, DEFAULT_VENDOR_ID, "Vendor ID should match mock value");
            assert_eq!(device_id.subsystem_vendor_id, DEFAULT_SUBSYSTEM_VENDOR_ID, "Subsystem vendor ID should match mock value");
            assert_eq!(device_id.subsystem_id, DEFAULT_SUBSYSTEM_ID, "Subsystem ID should match mock value");
...
fn test_get_device_id_different_device() {
    ...
    // Create and connect session
    let mut session = CaliptraSession::new(2, &mut mailbox_transport as &mut dyn caliptra_util_host_transport::Transport)
    ...
    match device_id_result {
        Ok(device_id) => {
            ...
            assert_eq!(device_id.device_id, expected_device_id);
            println!("Successfully verified different device ID via high-level API: 0x{:04X}", device_id.device_id);
...
fn test_get_device_id_session_not_connected() {
    ...
    let mut session = CaliptraSession::new(3, &mut mailbox_transport as &mut dyn caliptra_util_host_transport::Transport)
        .expect("Failed to create CaliptraSession");
    ...
    match device_id_result {
        ...
        Err(e) => {
            println!("GetDeviceId correctly failed with disconnected session: {:?}", e);
        }
    }
