...
use super::device_ownership_transfer::GetDotBackupBlobCmd;
...
pub fn get_command_handler(command_id: u32) -> Option<CommandHandlerFn> {
    match command_id {
        ...
        0x8024 => Some(process_command_with_metadata::<GetDotBackupBlobCmd>),
        _ => None,
    }
}
...
pub fn get_external_cmd_code(command_id: u32) -> Option<u32> {
    match command_id {
        ...
        0x8024 => Some(0x4D44_4F54), // GetDotBackupBlob -> MC_GET_DOT_BACKUP_BLOB ("MDOT")
        _ => None,
    }
}
...
mod tests {
    ...
    #[test]
    fn get_dot_backup_blob_is_registered() {
        let command_id = CaliptraCommandId::GetDotBackupBlob as u32;
        assert!(get_command_handler(command_id).is_some());
        assert_eq!(get_external_cmd_code(command_id), Some(0x4D44_4F54));
    }
}
