...
impl FromInternalRequest<GetAttestationRequest> for ExtCmdGetAttestationRequest {
    fn from_internal(internal: &GetAttestationRequest, command_code: u32) -> Self {
        ...
        Self {
            ...
            pki_entity_slot: internal.pki_entity_slot,
...
mod tests {
    ...
    /// The device decodes the request as a fixed-size struct and rejects any
    /// other length, so the size and field offsets are part of the wire
    /// contract. Keep this in step with `GetAttestationReq` in
    /// `common/mcu-mbox`.
    #[test]
    fn request_wire_layout_matches_the_device_struct() {
        use caliptra_mcu_core_util_host_command_types::attestation::{
            AsymAlgo, EvidenceFormat, PkiEntitySlot,
        };

        assert_eq!(core::mem::size_of::<ExtCmdGetAttestationRequest>(), 48);

        let nonce = [0x7Cu8; ATTESTATION_NONCE_LEN];
        let internal = GetAttestationRequest::new(
            EvidenceFormat::OcpEat,
            AsymAlgo::EccP384,
            PkiEntitySlot::Owner,
            &nonce,
        );
        let bytes = ExtCmdGetAttestationRequest::from_internal(&internal, 0x4D47_4154);
        let bytes = bytes.as_bytes();

        assert_eq!(bytes.len(), 48);
        assert_eq!(bytes[4..8], (EvidenceFormat::OcpEat as u32).to_le_bytes());
        assert_eq!(bytes[8..12], (AsymAlgo::EccP384 as u32).to_le_bytes());
        assert_eq!(bytes[12..16], (PkiEntitySlot::Owner as u32).to_le_bytes());
        assert_eq!(bytes[16..48], nonce);
    }
}
