...
pub fn hex_literal(val: u64) -> Literal {
    if val > 9 {
...
fn generate_register(reg: &RegisterType, ignore_rw: bool, no_super: bool) -> TokenStream {
    ...
    for field in reg.fields.iter() {
        ...
        let access_expr = if reg.width.in_bytes() * 8 == field.width as u64 {
            quote! { self.0 }
        ...
        let position_shift = if field.position > 0 {
            quote! { << #position }
        } else {
            TokenStream::new()
        };
        let (and_mask, not_mask) = if reg.width.in_bytes() * 8 == field.width as u64 {
            (TokenStream::new(), TokenStream::new())
        } else {
            (quote! { & #mask }, quote! { & !(#mask #position_shift) })
        };
        let comment = &field.comment.replace("<br>", "\n");
        let comment = comment.trim();
        if field.ty.can_read() || ignore_rw {
            if !comment.is_empty() {
            ...
            if field_ident == "len" {
                read_val_tokens.extend(quote! { #[allow(clippy::len_without_is_empty)] });
            }
            if let Some(ref enum_type) = field.enum_type {
                let enum_type_ident = camel_ident(enum_type.name.as_ref().unwrap());
                read_val_tokens.extend(quote! {
                    pub fn #field_ident(&self) -> #super_ enums::#enum_type_ident {
                        #super_ enums::#enum_type_ident::try_from(#access_expr).unwrap()
                    }
                });
        ...
        if field.ty.can_write() || ignore_rw {
            if !comment.is_empty() {
            ...
            if let Some(ref enum_type) = field.enum_type {
                ...
                write_val_tokens.extend(quote! {
                    pub fn #field_ident(self, f: impl FnOnce(#super_ enums::selector::#enum_selector_type) -> #super_ enums::#enum_type_ident) -> Self {
                        Self((self.0 #not_mask) | (#raw_type::from(f(#super_ enums::selector::#enum_selector_type())) #position_shift))
                    }
                });
            } else if field.width == 1 {
                write_val_tokens.extend(quote! {
                    pub fn #field_ident(self, val: bool) -> Self {
                        Self((self.0 #not_mask) | (#raw_type::from(val) #position_shift))
                    }
                });
            } else {
                write_val_tokens.extend(quote! {
                    pub fn #field_ident(self, val: #raw_type) -> Self {
                        Self((self.0 #not_mask) | ((val #and_mask) #position_shift))
                    }
                });
            }
        }
...
fn generate_register_types<'a>(regs: impl Iterator<Item = &'a RegisterType>) -> TokenStream {
    ...
    for reg in regs {
        if !has_single_32_bit_field(reg) {
            tokens.extend(generate_register(reg, false, false));
        }
    }
    tokens
}
...
fn read_write_types(
    ...
    if has_single_32_bit_field(t) {
        ...
        let read_type = read_val_ident(base_name, t);
        let write_type = write_val_ident(base_name, t);
